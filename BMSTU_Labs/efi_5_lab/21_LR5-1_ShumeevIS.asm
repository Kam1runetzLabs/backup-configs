...
cycle:
 ...
 ;       add     rcx, 8                                  ; Arg 1 - IN :
        ...
        mov     rbx,[BootServices]
        sub     rsp, 4*8                                ; "shadow space" for Rcx, Rdx, R8, R9
        call    qword [rbx + EFI_BOOT_SERVICES_TABLE.HandleProtocol]; OpenProtocol!
        add     rsp, 4*8
	...
	mov     rcx, [ScanCodeCount]	

	register_loop:
		mov rax, [ScanCodeCount]
		sub rax, rcx
		add rax, rax
		add rax, ScanCodes

		mov bx, word[rax]
		mov word[Key.ScanCode], bx
		push rcx
...
test_func:
	...
	;mov rdx, [BootServices]
	mov rcx, 0x7A120
	sub rsp, 5*8
	call qword[rdx + EFI_BOOT_SERVICES_TABLE.Stall]
	add rsp, 5*8 
